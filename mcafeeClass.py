...
def _get_content_type(filename):
	return 'application/xml'#mimetypes.guess_type(filename)[0] or 'application/octet-stream'
...
def _make_boundary():
	...
	pid = os.getppid()
	timestamp = time.time()
	randomstr = random.randrange(sys.maxsize)
	boundary = "{0}.{1}.{2}.{3}.{4}".format(ip,uid,pid,timestamp,randomstr)
	return boundary
...
class _CommandInvoker:
	...
	def __init__(self, host, port, username, password, protocol, output, display):
		...
		shandler = urllib.request.HTTPSHandler(context=gcontext)
		self.opener = urllib.request.build_opener(authhandler, urllib.request.HTTPCookieProcessor(),shandler)
	def save_token(self):
		"""
	...
	def invoke(self, command, args={}):
		...
		"""
		sys.stdout = open("resultssss.txt",'w')
		argscopy = args
	...
	def create_socket(self, url, args):
		...
		if( len(args) == 0):
			...
			bin_data = body.encode(_UTF8)
			data = 'Content-Type: ' + content_type + '\r\n'
			data += 'Content-Length: ' + str(len(bin_data)) + '\r\n\r\n'
			data += bin_data
			...
			log(logging.DEBUG, 'postdata:\r\n' + data)
			headers = {'Content-Type':content_type}
			bin_data = data.encode(_UTF8)
			req = urllib.request.Request(url, bin_data, headers)
			#print(url)
			#print(bin_data)
			#print(headers)
			return self.opener.open(req)'''
